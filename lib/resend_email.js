...
const resendKeyEmailSending = process.env.EMAIL_SENDING_DOMAIN_RESEND
const {APP_MAIN_URL} = require('../config/config');
const { convertToHtml } = require('./other_services');
    ...
    const { data, error } = await resend.emails.send({
        ...
        html: `
            ...
            <body>
                <div class="container">
                    <div class="header">
                        <img src="https://res.cloudinary.com/dcuhrkq08/image/upload/v1744879624/seefunnel-logo_1_utwpen.jpg" alt="Seenfunnel Logo">
                    </div>
                    <div class="content">
...
const sendEmailSendingAiResend = async (mailBox, prospectEmail, emailBody, subjectName) => {
    const resend = new Resend(resendKeyEmailSending);

    // Convert email body to HTML
    const htmlBody = convertToHtml(emailBody);
...
module.exports = {
    default : {sendEmail, sendEmailPlanSubscription, sendEmailPlanSubscriptionRenewal, sendEmailSendingAiResend, sendOnboardingEmailResend}
}