...
async function azureDetectLeadFinderFiltersInstruction(searchResponse, requestedFields = null) {
    try {
        ...
        // IMPORTANT: Provide a very explicit prompt to the model, explaining how to handle expansions.
        const question = `
You are an assistant that helps extract structured information from text requests. 
The user query or request is:
"""
${instruction}
"""

1) Extract only the explicitly mentioned details (if any):
   - firstName
   - lastName
   - industry
   - country (location)
   - keywords (technologies or topics mentioned)

2) Also, for certain broad or generic mentions, expand the keywords with possible relevant synonyms or related technologies. 
   For example:
     - If the text mentions AI, you can include expansions like "OpenAI", "ChatGPT", "Bard", "Anthropic", "Gemini", etc.
     - If the text mentions "Machine Learning," you can include synonyms like "ML", "Neural Networks," "Deep Learning," etc.
     - If the text mentions "Cloud," you might add expansions like AWS, Azure, GCP, etc. 
     - Feel free to infer other expansions if relevant from context.

3) If something is not mentioned, use "NOT_ABLE_TO_FIND" for that field.

4) Output must be **valid JSON** with the structure:
...
Make sure each field is present in the object. 
If multiple matches are found, you can return them in the "keywords" array or combine them.

Now, please generate the JSON response.
        `;

        // Call the Azure OpenAI Chat Completion
        const result = await client.chat.completions.create({
            messages: [
                {
                    role: "system",
                    content: "You are a structured data extraction assistant. Follow the instructions carefully."
                },
                {
                    role: "user",
                    content: question
        ...
        // Post-processing the returned data array
        // We validate the country and fill in any missing fields with "NOT_ABLE_TO_FIND" if needed.
        parsedResponse.data = parsedResponse.data.map(item => {
            return {
                firstName: item.firstName || "NOT_ABLE_TO_FIND",
                lastName: item.lastName || "NOT_ABLE_TO_FIND",
                industry: item.industry || "NOT_ABLE_TO_FIND",
